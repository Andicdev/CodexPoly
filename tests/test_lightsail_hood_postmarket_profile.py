...
_SEED = (
    ...
    / "025_add_hood_july_29_postmarket.sql"
)
_CHECK = (
    _ROOT / "deploy" / "lightsail" / "checks"
    / "verify_hood_july_29_postmarket.sql"
)
_ARM = (
    _ROOT / "deploy" / "lightsail" / "live"
    / "026_arm_hood_july_29_postmarket.sql"
)
_ARMED_CHECK = (
    _ROOT / "deploy" / "lightsail" / "checks"
    / "verify_hood_july_29_auto_live_armed.sql"
)


class RobinhoodPostmarketProfileSqlTests(unittest.TestCase):
    def test_seed_is_hood_only_and_non_live(self) -> None:
        text = _SEED.read_text(encoding="utf-8")
        upper = text.upper()

        self.assertIn("'earnings:HOOD:2026Q2'", text)
        for ticker in ("QCOM", "MSFT", "META", "EA", "EBAY"):
