...
_SEED = (
    ...
    / "lightsail"
    / "seeds"
    / "022_add_msft_july_29_postmarket.sql"
)
_CHECK = (
    _ROOT
    / "deploy"
    / "lightsail"
    / "checks"
    / "verify_msft_july_29_postmarket.sql"
)
_ARM = (
    _ROOT
    / "deploy"
    / "lightsail"
    / "live"
    / "023_arm_msft_july_29_postmarket.sql"
)
_ARMED_CHECK = (
    _ROOT
    / "deploy"
    / "lightsail"
    / "checks"
    / "verify_msft_july_29_auto_live_armed.sql"
)


class MicrosoftPostmarketProfileSqlTests(unittest.TestCase):
    def test_seed_is_msft_only_and_non_live(self) -> None:
        text = _SEED.read_text(encoding="utf-8")
        upper = text.upper()

        self.assertIn("'earnings:MSFT:2026Q4'", text)
        for ticker in ("QCOM", "META", "EBAY", "HOOD", "SBUX"):
    ...
    def test_seed_declares_three_source_paths(self) -> None:
        ...
        self.assertIn('"allowed_document_hosts": ["www.microsoft.com"]', text)
