...
_SEED = (
    ...
    / "lightsail"
    / "seeds"
    / "024_add_ea_july_29_postmarket.sql"
)
_CHECK = (
    _ROOT
    / "deploy"
    / "lightsail"
    / "checks"
    / "verify_ea_july_29_postmarket.sql"
)
_ARM = (
    _ROOT
    / "deploy"
    / "lightsail"
    / "live"
    / "025_arm_ea_july_29_postmarket.sql"
)
_ARMED_CHECK = (
    _ROOT
    / "deploy"
    / "lightsail"
    / "checks"
    / "verify_ea_july_29_auto_live_armed.sql"
)


class ElectronicArtsPostmarketProfileSqlTests(unittest.TestCase):
    def test_seed_is_ea_only_and_non_live(self) -> None:
        text = _SEED.read_text(encoding="utf-8")
        upper = text.upper()

        self.assertIn("'earnings:EA:2027Q1'", text)
    ...
    def test_seed_declares_all_source_paths(self) -> None:
        ...
        self.assertIn("market_active_no_official_call", text)
