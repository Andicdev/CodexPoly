...
_SEED = (
    ...
    / "lightsail"
    / "seeds"
    / "031_add_amzn_july_30_postmarket.sql"
)
_CHECK = (
    _ROOT
    / "deploy"
    / "lightsail"
    / "checks"
    / "verify_amzn_july_30_postmarket.sql"
)
_ARM = (
    _ROOT
    / "deploy"
    / "lightsail"
    / "live"
    / "037_arm_amzn_july_30_postmarket.sql"
)
_ARMED_CHECK = (
    _ROOT
    / "deploy"
    / "lightsail"
    / "checks"
    / "verify_amzn_july_30_auto_live_armed.sql"
)


class AmazonPostmarketProfileSqlTests(unittest.TestCase):
    def test_seed_is_amzn_only_and_non_live(self) -> None:
        text = _SEED.read_text(encoding="utf-8")
        upper = text.upper()

        self.assertIn("'earnings:AMZN:2026Q2'", text)
        self.assertIn("'amzn-2026q2-gaap-eps-1pt82'", text)
    ...
    def test_seed_declares_all_parallel_source_paths(self) -> None:
        text = _SEED.read_text(encoding="utf-8")
