-- Add only the reviewed July 29 HOOD POST_MARKET event. The profile remains
...
BEGIN
    IF now() >= TIMESTAMPTZ '2026-07-29 17:45:00+00' THEN
        RAISE EXCEPTION 'HOOD profile preparation deadline has passed';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'HOOD scope already contains facts or claims';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'HOOD market has an active order group';
    END IF;
END
$guard$;
...
VALUES (
    ...
    '{
        ...
        "company_ir": {
            "allowed_document_hosts": ["investors.robinhood.com"],
            "feed_url": "https://investors.robinhood.com/rss/news-releases.xml",
            "kind": "rss",
            "provider": "company_ir",
            "title_all": [
                "Robinhood",
...
VALUES (
    ...
    'earnings:HOOD:2026Q2',
    'earnings_resolution',
    'https://polymarket.com/event/hood-quarterly-earnings-gaap-eps-07-29-2026-0pt43',
    'abccbaq',
    '0x00d480ad192a0cf494a9663a8d0fe22578b06ea4702f83acfb79bde049a5cf85',
    0.999,
    0.999,
    ...
    '{
        ...
        "rule_key": "hood-2026q2-gaap-eps-0pt43",
        "ticker": "HOOD",
...
VALUES (
    ...
    'CONFIRMED',
    'https://investors.robinhood.com/news-releases/news-release-details/robinhood-markets-inc-announce-second-quarter-2026-results-july',
    'PARSER_ONLY',
    'FULL_HTML',
    '{
        "comparison_op": ">",
    ...
    'Official Q2 release and 21:00 UTC call confirmed; parser replayed against real Q1 2026 HTML.',
    now()
)
...
VALUES (
    ...
    TIMESTAMPTZ '2026-07-30 02:00:00+00',
    '{
        "seed": "025_add_hood_july_29_postmarket",
...
BEGIN
    IF (
        ...
        RAISE EXCEPTION 'HOOD rule verification failed';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION 'HOOD profile verification failed';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION 'HOOD schedule verification failed';
    END IF;
    ...
    IF reviewed_notional <> 99.9 OR reviewed_notional > 1000 THEN
        RAISE EXCEPTION 'HOOD reviewed notional is invalid';
    END IF;
END
$verify$;
