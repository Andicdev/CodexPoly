-- Arm only AMZN for its reviewed July 30 POST_MARKET window.
...
BEGIN
    IF now() >= TIMESTAMPTZ '2026-07-30 18:00:00+00' THEN
        RAISE EXCEPTION 'AMZN live arming deadline has passed';
    END IF;
    ...
    WHERE schedule_key = 'schedule:earnings-amzn-2026q2'
      ...
      AND preflight_at = TIMESTAMPTZ '2026-07-30 17:45:00+00'
      AND activate_at = TIMESTAMPTZ '2026-07-30 18:00:00+00'
    ...
    IF schedule_state IS NULL THEN
        RAISE EXCEPTION 'AMZN reviewed schedule is missing';
    END IF;

    IF now() >= TIMESTAMPTZ '2026-07-30 17:45:00+00'
       AND (
    ...
    THEN
        RAISE EXCEPTION 'AMZN authenticated readiness is not fresh';
    END IF;
    ...
    IF (
        ...
        WHERE profile.profile_key = 'earnings-amzn-2026q2'
          AND profile.scope_id = 'earnings:AMZN:2026Q2'
          ...
          AND rule.rule_key = 'amzn-2026q2-gaap-eps-1pt82'
          AND rule.status = 'SHADOW'
    ) <> 1 THEN
        RAISE EXCEPTION 'reviewed AMZN disabled profile is invalid';
    END IF;
    ...
    SET
        ...
        metadata = metadata || jsonb_build_object(
            'armed_for_live', true,
            'armed_by', '037_arm_amzn_july_30_postmarket',
...
BEGIN
    IF (
        ...
        RAISE EXCEPTION 'AMZN live arming verification failed';
    END IF;
END
$verify$;
