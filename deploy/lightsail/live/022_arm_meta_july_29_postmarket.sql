-- Arm only META for the July 29 POST_MARKET window. This migration never
-- enables the execution profile directly; scheduler activation still
-- requires fresh authenticated readiness evidence.
...
BEGIN
    IF now() >= TIMESTAMPTZ '2026-07-29 18:00:00+00' THEN
        RAISE EXCEPTION 'META live arming deadline has passed';
    END IF;
    ...
    WHERE schedule_key = 'schedule:earnings-meta-2026q2'
      ...
      AND metadata ->> 'block_id' = '2026-07-29-meta-post-market';

    IF schedule_state IS NULL THEN
        RAISE EXCEPTION 'META reviewed schedule is missing';
    END IF;
    ...
    IF (
        ...
        WHERE profile.profile_key = 'earnings-meta-2026q2'
          AND profile.scope_id = 'earnings:META:2026Q2'
          ...
          AND rule.rule_key = 'meta-2026q2-gaap-eps-7pt20'
          AND rule.status = 'SHADOW'
    ) <> 1 THEN
        RAISE EXCEPTION 'META reviewed disabled profile is invalid';
    END IF;
    ...
    SET
        ...
        metadata = metadata || jsonb_build_object(
            'armed_for_live', true,
            'armed_by', '022_arm_meta_july_29_postmarket'
        ),
        updated_at = now()
    WHERE schedule_key = 'schedule:earnings-meta-2026q2';
END
...
BEGIN
    IF (
        ...
        RAISE EXCEPTION 'META live arming verification failed';
    END IF;
END
$verify$;
