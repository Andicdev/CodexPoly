-- Arm only HOOD for the July 29 POST_MARKET window. This migration never
...
BEGIN
    IF now() >= TIMESTAMPTZ '2026-07-29 18:00:00+00' THEN
        RAISE EXCEPTION 'HOOD live arming deadline has passed';
    END IF;
    ...
    IF schedule_state IS NULL THEN
        RAISE EXCEPTION 'HOOD reviewed schedule is missing';
    END IF;
    ...
    IF (
        ...
        WHERE profile.profile_key = 'earnings-hood-2026q2'
          AND profile.scope_id = 'earnings:HOOD:2026Q2'
          ...
          AND rule.rule_key = 'hood-2026q2-gaap-eps-0pt43'
          AND rule.status = 'SHADOW'
    ) <> 1 THEN
        RAISE EXCEPTION 'HOOD reviewed disabled profile is invalid';
    END IF;
    ...
    SET
        ...
        metadata = metadata || jsonb_build_object(
            'armed_for_live', true,
            'armed_by', '026_arm_hood_july_29_postmarket'
        ),
        updated_at = now()
    WHERE schedule_key = 'schedule:earnings-hood-2026q2';
END
...
BEGIN
    IF (
        ...
        RAISE EXCEPTION 'HOOD live arming verification failed';
    END IF;
END
$verify$;
