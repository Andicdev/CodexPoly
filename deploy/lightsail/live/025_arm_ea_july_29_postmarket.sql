-- Arm only EA for the July 29 POST_MARKET window. This migration never
...
BEGIN
    IF now() >= TIMESTAMPTZ '2026-07-29 18:00:00+00' THEN
        RAISE EXCEPTION 'EA live arming deadline has passed';
    END IF;
    ...
    IF schedule_state IS NULL THEN
        RAISE EXCEPTION 'EA reviewed schedule is missing';
    END IF;
    ...
    IF (
        ...
        WHERE profile.profile_key = 'earnings-ea-2027q1'
          AND profile.scope_id = 'earnings:EA:2027Q1'
          ...
          AND rule.rule_key = 'ea-2027q1-gaap-eps-0pt80'
          AND rule.status = 'SHADOW'
    ) <> 1 THEN
        RAISE EXCEPTION 'EA reviewed disabled profile is invalid';
    END IF;
    ...
    SET
        ...
        metadata = metadata || jsonb_build_object(
            'armed_for_live', true,
            'armed_by', '025_arm_ea_july_29_postmarket'
        ),
        updated_at = now()
    WHERE schedule_key = 'schedule:earnings-ea-2027q1';
END
...
BEGIN
    IF (
        ...
        RAISE EXCEPTION 'EA live arming verification failed';
    END IF;
END
$verify$;
