-- Arm only AAPL for its reviewed July 30 POST_MARKET window.
...
BEGIN
    ...
    WHERE schedule_key = 'schedule:earnings-aapl-2026q3'
      AND profile_key = 'earnings-aapl-2026q3'
      AND automation_mode = 'AUTO_PREFLIGHT'
      AND state IN ('PENDING', 'READY')
      AND preflight_at = TIMESTAMPTZ '2026-07-30 18:15:00+00'
      AND activate_at = TIMESTAMPTZ '2026-07-30 18:30:00+00'
    ...
    IF schedule_state IS NULL THEN
        RAISE EXCEPTION 'AAPL reviewed schedule is missing';
    END IF;

    IF now() >= TIMESTAMPTZ '2026-07-30 18:15:00+00'
       AND (
    ...
    THEN
        RAISE EXCEPTION 'AAPL authenticated readiness is not fresh';
    END IF;
    ...
    IF (
        ...
        WHERE profile.profile_key = 'earnings-aapl-2026q3'
          ...
          AND rule.rule_key = 'aapl-2026q3-gaap-eps-1pt89'
          AND rule.status = 'SHADOW'
    ) <> 1 THEN
        RAISE EXCEPTION 'reviewed AAPL disabled profile is invalid';
    END IF;
...
BEGIN
    IF (
        ...
        RAISE EXCEPTION 'AAPL live arming verification failed';
    END IF;
END
$verify$;
