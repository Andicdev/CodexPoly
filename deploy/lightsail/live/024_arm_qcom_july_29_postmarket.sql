-- Arm only QCOM for the July 29 POST_MARKET window. This migration never
...
BEGIN
    IF now() >= TIMESTAMPTZ '2026-07-29 18:00:00+00' THEN
        RAISE EXCEPTION 'QCOM live arming deadline has passed';
    END IF;
    ...
    IF schedule_state IS NULL THEN
        RAISE EXCEPTION 'QCOM reviewed schedule is missing';
    END IF;
    ...
    IF (
        ...
        WHERE profile.profile_key = 'earnings-qcom-2026q3'
          AND profile.scope_id = 'earnings:QCOM:2026Q3'
          ...
          AND rule.rule_key = 'qcom-2026q3-nongaap-eps-2pt23'
          AND rule.status = 'SHADOW'
    ) <> 1 THEN
        RAISE EXCEPTION 'QCOM reviewed disabled profile is invalid';
    END IF;
    ...
    SET
        ...
        metadata = metadata || jsonb_build_object(
            'armed_for_live', true,
            'armed_by', '024_arm_qcom_july_29_postmarket'
        ),
        updated_at = now()
    WHERE schedule_key = 'schedule:earnings-qcom-2026q3';
END
...
BEGIN
    IF (
        ...
        RAISE EXCEPTION 'QCOM live arming verification failed';
    END IF;
END
$verify$;
