-- Arm only DLB for its reviewed July 30 POST_MARKET window.
...
BEGIN
    IF now() >= TIMESTAMPTZ '2026-07-30 18:15:00+00' THEN
        RAISE EXCEPTION 'DLB live arming deadline has passed';
    END IF;
    ...
    WHERE schedule_key = 'schedule:earnings-dlb-2026q3'
      ...
      AND preflight_at = TIMESTAMPTZ '2026-07-30 18:00:00+00'
      AND activate_at = TIMESTAMPTZ '2026-07-30 18:15:00+00'
    ...
    IF schedule_state IS NULL THEN
        RAISE EXCEPTION 'DLB reviewed schedule is missing';
    END IF;

    IF now() >= TIMESTAMPTZ '2026-07-30 18:00:00+00'
       AND (
    ...
    THEN
        RAISE EXCEPTION 'DLB authenticated readiness is not fresh';
    END IF;
    ...
    IF (
        ...
        WHERE profile.profile_key = 'earnings-dlb-2026q3'
          AND profile.scope_id = 'earnings:DLB:2026Q3'
          ...
          AND rule.rule_key = 'dlb-2026q3-nongaap-eps-0pt67'
          AND rule.status = 'SHADOW'
    ) <> 1 THEN
        RAISE EXCEPTION 'reviewed DLB disabled profile is invalid';
    END IF;
    ...
    SET
        ...
        metadata = metadata || jsonb_build_object(
            'armed_for_live', true,
            'armed_by', '039_arm_dlb_july_30_postmarket',
...
BEGIN
    IF (
        ...
        RAISE EXCEPTION 'DLB live arming verification failed';
    END IF;
END
$verify$;
