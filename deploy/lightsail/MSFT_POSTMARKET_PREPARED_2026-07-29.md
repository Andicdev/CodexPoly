...
`deploy/lightsail/live/023_arm_msft_july_29_postmarket.sql` was applied to
production after explicit authorization with caps `100 / 100 / 1000`.

The immediate read-only observation at `2026-07-29T16:16:25Z` confirmed:

- schedule mode/state: `AUTO_LIVE / PENDING`;
- profile status: `DISABLED`;
- `armed_for_live=true`;
- live heartbeat age: 1 second;
...
evidence. The profile-gated Microsoft IR and SEC-current polling transports
start only after that activation.
