-- Read-only pre-activation check for the RDDT AUTO_LIVE schedule.
...
BEGIN
    IF now() >= TIMESTAMPTZ '2026-07-30 18:08:00+00' THEN
        RAISE EXCEPTION 'RDDT armed check is only valid before activation';
    END IF;
    ...
    WHERE schedule.schedule_key = 'schedule:earnings-rddt-2026q2'
      AND schedule.profile_key = 'earnings-rddt-2026q2'
      AND schedule.automation_mode = 'AUTO_LIVE'
      AND schedule.activate_at = TIMESTAMPTZ '2026-07-30 18:08:00+00'
    ...
    IF schedule_state IS NULL OR profile_state <> 'DISABLED' THEN
        RAISE EXCEPTION 'RDDT armed schedule or disabled profile is invalid';
    END IF;
    ...
    IF now() < TIMESTAMPTZ '2026-07-30 17:53:00+00' THEN
        ...
        THEN
            RAISE EXCEPTION 'RDDT state is invalid before preflight';
        END IF;
    ELSE
