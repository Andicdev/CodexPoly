-- Verify the complete July 29 PRE_MARKET authenticated-preflight result
-- without returning profile, account, market, readiness, or order data.
...
BEGIN
    IF now() < TIMESTAMPTZ '2026-07-29 08:45:00+00'
       OR now() >= TIMESTAMPTZ '2026-07-29 09:00:00+00' THEN
        RAISE EXCEPTION
            'July 29 PRE_MARKET readiness check is outside its window';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION
            'July 29 PRE_MARKET authenticated preflight is not fully ready';
    END IF;
