-- Read-only pre-activation check for the DLB AUTO_LIVE schedule.
...
BEGIN
    IF now() >= TIMESTAMPTZ '2026-07-30 18:15:00+00' THEN
        RAISE EXCEPTION 'DLB armed check is only valid before activation';
    END IF;
    ...
    WHERE schedule.schedule_key = 'schedule:earnings-dlb-2026q3'
      AND schedule.profile_key = 'earnings-dlb-2026q3'
    ...
    IF schedule_state IS NULL OR profile_state <> 'DISABLED' THEN
        RAISE EXCEPTION 'DLB armed schedule or disabled profile is invalid';
    END IF;
    ...
    IF now() < TIMESTAMPTZ '2026-07-30 18:00:00+00' THEN
        ...
        THEN
            RAISE EXCEPTION 'DLB state is invalid before preflight';
        END IF;
    ELSE
