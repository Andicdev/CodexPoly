-- Read-only check for AMZN after scheduler-owned activation and before signal.
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION 'AMZN active check is outside its valid interval';
    END IF;
    ...
    IF (
        ...
        WHERE schedule.schedule_key = 'schedule:earnings-amzn-2026q2'
          ...
          AND profile.profile_key = 'earnings-amzn-2026q2'
        ...
        RAISE EXCEPTION 'AMZN active schedule or enabled profile is invalid';
    END IF;
