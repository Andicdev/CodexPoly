-- Read-only pre-activation check for the AMZN AUTO_LIVE schedule.
...
BEGIN
    IF now() >= TIMESTAMPTZ '2026-07-30 18:00:00+00' THEN
        RAISE EXCEPTION 'AMZN armed check is only valid before activation';
    END IF;
    ...
    WHERE schedule.schedule_key = 'schedule:earnings-amzn-2026q2'
      AND schedule.profile_key = 'earnings-amzn-2026q2'
    ...
    IF schedule_state IS NULL OR profile_state <> 'DISABLED' THEN
        RAISE EXCEPTION 'AMZN armed schedule or disabled profile is invalid';
    END IF;
    ...
    IF now() < TIMESTAMPTZ '2026-07-30 17:45:00+00' THEN
        ...
        THEN
            RAISE EXCEPTION 'AMZN state is invalid before preflight';
        END IF;
    ELSE
