...
DO $verify$
BEGIN
    IF now() < TIMESTAMPTZ '2026-07-30 18:45:00+00'
       OR now() >= TIMESTAMPTZ '2026-07-30 20:00:00+00'
    THEN
        RAISE EXCEPTION 'RIVN active check is outside its valid interval';
    END IF;
    IF (
        SELECT count(*)
        ...
        WHERE schedule.schedule_key =
          ...
          AND profile.profile_key = 'earnings-rivn-2026q2'
          AND profile.status = 'ENABLED'
          AND profile.quantity = 100
    ) <> 1 THEN
        RAISE EXCEPTION 'RIVN active state is invalid';
    END IF;
    IF NOT EXISTS (
