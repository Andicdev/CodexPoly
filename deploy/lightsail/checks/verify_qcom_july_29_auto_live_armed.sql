-- Verify the armed QCOM schedule before its activation boundary without
...
BEGIN
    IF now() >= TIMESTAMPTZ '2026-07-29 18:00:00+00' THEN
        RAISE EXCEPTION 'QCOM armed check is only valid before activation';
    END IF;
    ...
    WHERE schedule.schedule_key = 'schedule:earnings-qcom-2026q3'
      AND schedule.profile_key = 'earnings-qcom-2026q3'
    ...
    IF schedule_state IS NULL OR profile_state <> 'DISABLED' THEN
        RAISE EXCEPTION 'QCOM armed schedule or disabled profile is invalid';
    END IF;
    ...
    IF now() < TIMESTAMPTZ '2026-07-29 17:45:00+00' THEN
        ...
        THEN
            RAISE EXCEPTION 'QCOM state is invalid before preflight';
        END IF;
    ELSE
        IF schedule_state <> 'READY'
           ...
           OR readiness_until <=
               TIMESTAMPTZ '2026-07-29 18:00:00+00'
        THEN
            RAISE EXCEPTION 'QCOM authenticated readiness is not fresh';
        END IF;
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'QCOM source rule is missing';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'QCOM active order group already exists';
    END IF;
    ...
    IF reviewed_notional <> 99.9 OR reviewed_notional > 1000 THEN
        RAISE EXCEPTION 'QCOM armed notional is invalid';
    END IF;
END
$verification$;
