...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Check or explicitly apply the additive source notification "
            "outbox schema."
        )
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Explicitly apply migration 010. Production should use the "
            ...
            json.dumps(
                {
                    ...
                    "error": redact_exception(
                        RuntimeError(
                            "Notification outbox schema operation failed: "
                            f"{type(exc).__name__}"
                        )
                    ),
                }
