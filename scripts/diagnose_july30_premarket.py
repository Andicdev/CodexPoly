...
WHERE profile.profile_key = ANY(CAST(:profile_keys AS text[]))
  AND event.received_at >= TIMESTAMPTZ '2026-07-30 09:30:00+00'
...
SELECT
    ...
    claim.result -> 'order_ids' ->> 0 AS order_id
FROM resolution_execution_profiles AS profile
JOIN resolution_execution_claims AS claim
  ON claim.scope_id = profile.scope_id
WHERE profile.profile_key = ANY(CAST(:profile_keys AS text[]))
  AND claim.status = 'EXECUTED'
  AND claim.result -> 'order_ids' ->> 0 IS NOT NULL
ORDER BY profile.profile_key, claim.created_at
""".strip()
...
def main() -> int:
    ...
    try:
        ...
        _emit("PREMARKET_SUMMARY", summaries)
        _emit("PREMARKET_SOURCE", sources)
        _emit("PREMARKET_ORDER", orders)
        _emit("PREMARKET_JOURNAL", journals)
        _emit("PREMARKET_SUPERVISION", supervision)
        ...
        for row in remote_orders:
            ...
            if inspection.failed_order_ids:
                ...
                payload = {
                    ...
                    "remote_status": snapshot.raw_status,
